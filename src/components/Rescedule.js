...
export default function Rescedule() {
    ...
    useEffect(()=>{
        if(id){
     setbookingdetails({...bookingdetails,id})
        }
    },[id])
    ...
    const handledata = async () => {
      ...
      if (isvalid) {
        ...
        try {
          const response = await axios.patch('http://localhost:5000/booking/reseducleslot', {bookingdetails}, { headers: { 'Content-Type': 'application/json' } })
