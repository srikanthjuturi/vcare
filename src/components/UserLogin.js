...
export default function UserLogin() {
  ...
  const submitdata = async () => {
    ...
    if (isvalid) {
      try {
        const response = await axios.post('http://localhost:5000/user/login', { logindata: logindata }, { headers: { 'Content-Type': 'application/json' } })
    ...
    <>
      ...
      <div className='doctor-login-parent'>
        ...
        <div className='doctor-login-inner'>
          <h5 className='text-center'><span><img src={user} alt="" className='doctor-img-in-register' /></span> User Profile</h5>
          <div>
